...
use x509_parser::{
	...
	extensions::{ParsedExtension, SubjectAlternativeName, GeneralName},
	traits::FromDer,
};
...
impl ParsedCertificate {
	pub(crate) fn from_der(der: Cow<'_, Vec<u8>>) -> io::Result<Self> {
		...
		let parsed: X509Certificate<'_> = match X509Certificate::from_der(&lifetime_bound[..]) {
			Ok((_, parsed)) => parsed,
			Err(nom::Err::Failure(e)) | Err(nom::Err::Error(e)) => return Err(io::Error::new(io::ErrorKind::InvalidData, e)),
			Err(nom::Err::Incomplete(_)) => return Err(io::Error::new(io::ErrorKind::UnexpectedEof, "incomplete X.509 object")),
		};
...
impl LuaUserData for ParsedCertificate {
	fn add_methods<'l, M: LuaUserDataMethods<'l, Self>>(methods: &mut M) {
		methods.add_method("extensions", |_, this, _: ()| -> LuaResult<ExtensionsHandle> {
			Ok(ExtensionsHandle(this.clone()))
		});
		...
		methods.add_method("subject", |lua, this, _: ()| -> LuaResult<LuaTable> {
			...
			let oid = oid_registry::OID_X509_COMMON_NAME.to_id_string().to_lua(lua)?;
			let mut cns: Vec<&str> = this.parsed().subject().iter_common_name().filter_map(|x| {
				x.as_str().ok()
			}).collect();
...
impl LuaUserData for ExtensionsHandle {
	fn add_methods<'l, M: LuaUserDataMethods<'l, Self>>(methods: &mut M) {
		methods.add_meta_method(LuaMetaMethod::Index, |lua, this, oid: LuaString| -> LuaResult<Option<LuaAnyUserData<'l>>> {
...
impl LuaUserData for SANHandle {
	fn add_methods<'l, M: LuaUserDataMethods<'l, Self>>(methods: &mut M) {
		methods.add_meta_method(LuaMetaMethod::Index, |lua, this, key: LuaString| -> LuaResult<Option<LuaTable>> {
			...
			let mut names: Vec<&[u8]> = match key {
				"dNSName" => iter.filter_map(|n| {
					match n {
						GeneralName::DNSName(n) => Some(n.as_bytes()),
						_ => None,
					}
				}).collect(),
				"uniformResourceIdentifier" => iter.filter_map(|n| {
					match n {
