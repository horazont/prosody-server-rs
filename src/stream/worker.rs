...
use crate::ioutil::{iotimeout, AsyncReadable, Duplex, DuplexResult, ReadResult};
use crate::tls;
use crate::verify;

use super::handle::AddrStr;
use super::msg::{ControlMessage, SocketOption};
...
impl fmt::Debug for Stream {
	fn fmt<'f>(&self, f: &'f mut fmt::Formatter) -> fmt::Result {
		match self {
			...
			Self::TlsTcpServer { .. } => f
				.debug_struct("Stream::TlsTcpServer")
				.finish_non_exhaustive(),
			Self::TlsTcpClient { .. } => f
				.debug_struct("Stream::TlsTcpClient")
				.finish_non_exhaustive(),
			Self::TlsUnixServer { .. } => f
				.debug_struct("Stream::TlsUnixServer")
				.finish_non_exhaustive(),
			Self::TlsUnixClient { .. } => f
				.debug_struct("Stream::TlsUnixClient")
				.finish_non_exhaustive(),
		}
	}
}
...
impl Stream {
	fn broken_err(e: &Option<Box<dyn std::error::Error + Send + 'static>>) -> io::Error {
		match e {
			Some(e) => io::Error::new(
				...
				format!(
					"connection invalidated because of a previous failed operation: {}",
					e
				),
			),
			None => io::Error::new(
				io::ErrorKind::ConnectionReset,
				"connection invalidated because of a previous failed operation (unknown error)",
			),
		}
	}
	...
	where
		server::TlsStream<T>: Into<Self>,
	{
		let (verify, sock) = recorder
			.scope(iotimeout(
				handshake_timeout,
		...
		client::TlsStream<T>: Into<Self>,
	{
		...
		match sock {
			...
			Err(e) => {
				...
				*self = Self::Broken {
					e: Some(Box::new(opaque(format!(
						"failed to initiate TLS connection: {}",
						e
					)))),
				};
				Err(e)
	...
	async fn starttls_connect(
		...
		match tmp {
			...
			| Self::TlsTcpClient { .. }
			| Self::TlsUnixServer { .. }
			| Self::TlsUnixClient { .. } => Err(io::Error::new(
				io::ErrorKind::InvalidInput,
				"attempt to start TLS on a socket with TLS",
			)),
			Self::PlainTcp { conn } => {
				self.starttls_client(conn, name, ctx.into(), recorder, handshake_timeout)
					.await
			}
			Self::PlainUnix { conn } => {
				self.starttls_client(conn, name, ctx.into(), recorder, handshake_timeout)
	...
	async fn starttls_accept(
		...
		match tmp {
			...
			Self::PlainTcp { conn } => {
				self.starttls_server(conn, ctx.into(), recorder, handshake_timeout)
					.await
			}
			Self::PlainUnix { conn } => {
				self.starttls_server(conn, ctx.into(), recorder, handshake_timeout)
...
impl AnyStream {
	pub(super) fn try_from_raw_fd(fd: RawFd) -> io::Result<Self> {
		...
		match sock.domain()? {
			socket2::Domain::IPV4 | socket2::Domain::IPV6 => Ok(Self::Tcp(sock.into())),
			socket2::Domain::UNIX => Ok(Self::Unix(sock.into())),
			_ => Err(io::Error::new(
				io::ErrorKind::InvalidInput,
	...
	async fn proc_msg(&mut self, msg: ControlMessage) -> io::Result<MsgResult> {
		match msg {
			...
			ControlMessage::AcceptTls(ctx, recorder) => {
				...
				let tls_info = self
					.get_fdstream_mut()
					.starttls_accept(ctx, &recorder, timeout)
					.await?;
				match MAIN_CHANNEL
			...
			ControlMessage::ConnectTls(name, ctx, recorder) => {
				...
				let tls_info = self
					.get_fdstream_mut()
					.starttls_connect(name, ctx, &*recorder, timeout)
					.await?;
				match MAIN_CHANNEL
					.send(Message::TlsStarted {
						handle: self.handle.clone(),
