...
use super::lua::{set_listeners, to_servername};
use super::msg::{ControlMessage, SocketOption};
use super::state::StreamState;

pub(super) enum AddrStr {
	Unspecified,
	InetAny { addr: String, port: u16 },
	Unix { path: String },
}
...
impl LuaUserData for StreamHandle {
	fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
		...
		methods.add_method(
			"clientport",
			|_, this: &Self, _: ()| -> LuaResult<Option<u16>> {
				match this.kind {
		...
		methods.add_method(
			"ssl_info",
			|lua, this: &Self, _: ()| -> LuaResult<Option<LuaTable>> {
				match &this.state {
		...
		methods.add_method(
			"ssl_peercertificate",
			|_, this, _: ()| -> LuaResult<Option<cert::ParsedCertificate>> {
				match &this.state {
					StreamState::Tls { info, .. } => match &info.verify {
						verify::VerificationRecord::Unverified
						| verify::VerificationRecord::Failed { .. } => Ok(None),
						verify::VerificationRecord::Passed { cert: certificate } => Ok(
							cert::ParsedCertificate::from_der(Cow::Borrowed(&certificate.0)).ok(),
						),
					},
					_ => Ok(None),
				}
		...
		methods.add_method(
			"ssl_peerverification",
			|lua, this: &Self, _: ()| -> LuaResult<(bool, LuaTable)> {
				...
				match &this.state {
					StreamState::Tls { info, .. } => match &info.verify {
						...
						verify::VerificationRecord::Passed { .. } => Ok((true, reasons)),
		...
		methods.add_method(
			"setoption",
			|lua, this: &Self, name: String| -> LuaResult<(bool, Option<String>)> {
		...
		methods.add_method_mut(
			...
			|_,
			 this: &mut Self,
			 (ctx, servername): (Option<tls::TlsConfigHandle>, Option<LuaString>)|
			 -> LuaResult<()> {
		...
		methods.add_method(
			"write",
			|_, this: &Self, data: LuaString| -> LuaResult<usize> {
		...
		methods.add_function(
			"setlistener",
			|_, (this, listeners, data): (LuaAnyUserData, LuaTable, LuaValue)| -> LuaResult<()> {
		...
		methods.add_meta_function(
			LuaMetaMethod::Index,
			|_, (this, key): (LuaAnyUserData, LuaString)| -> LuaResult<LuaValue> {
		...
		methods.add_meta_function(
			...
			|_, (this, key, value): (LuaAnyUserData, LuaString, LuaValue)| -> LuaResult<()> {
				let data = this.get_user_value::<LuaTable>()?;
